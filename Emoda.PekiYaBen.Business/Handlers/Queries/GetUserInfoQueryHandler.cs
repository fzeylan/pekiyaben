...
{
    public class GetUserInfoQueryHandler : IQueryHandler<GetUserInfoQuery, QuerySingleResult<AppUserInfo>>
    {
        public QuerySingleResult<AppUserInfo> Retrieve(GetUserInfoQuery query)
        {
            using (IDbConnection con = new System.Data.SqlClient.SqlConnection(ConfigurationManager.ConnectionStrings["PekiYaBen"].ConnectionString))
            {
                ...
                {
                    var userInfo = con.Query<AppUserInfo>("SELECT * FROM AdminUser WHERE Id = @Id", query, trans).SingleOrDefault();
